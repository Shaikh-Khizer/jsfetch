...
func (sf *SkipFilter) ShouldSkip(rawURL string, baseHost string) (bool, string) {
	...
	allDomains := make([]string, len(defaultSkipDomains)+len(sf.ExtraDomains))
	copy(allDomains, defaultSkipDomains)
	copy(allDomains[len(defaultSkipDomains):], sf.ExtraDomains)
	...
	allPatterns := make([]string, len(defaultSkipPatterns)+len(sf.ExtraPatterns))
	copy(allPatterns, defaultSkipPatterns)
	copy(allPatterns[len(defaultSkipPatterns):], sf.ExtraPatterns)
	extra := []string{
		"vendor",
		"bundle",
		"chunk",
