...
func main() {
	...
	flag.StringVar(statusRangeS, "as", "200-299", "Shorthand for --accept-status")
	...
	followRedir := flag.Bool("r", false, "Follow redirects (default: off — redirects are reported and aborted)")
	...
	*method = strings.ToUpper(strings.TrimSpace(*method))
	...
	client 		 := newHTTPClient(*timeout, proxyURL, *insecure, *followRedir)
	...
	if *followRedir {
		log("[~] Redirects    : follow (-r)")
	} else {
		log("[~] Redirects    : off (pass -r to follow)")
	}
	if *method != "GET" {
	...
	log("[*] Page status  : %d %s", resp.StatusCode, http.StatusText(resp.StatusCode))
	...
	for _, r := range results {
		if r.Err != nil {
			...
			log("[✓] Saved  %s  (%d bytes)", r.URL, r.Bytes)
			successCount++
		}
	}
	...
	if !*doInline {
		totalSkipped := make([]SkippedItem, len(skippedJS)+len(skippedLinks))
		copy(totalSkipped, skippedJS)
		copy(totalSkipped[len(skippedJS):], skippedLinks)
	...
	log("\n[*] Inline JS : %d/%d saved", inlineCount, len(inlines))
...
func printUsage() {
	...
	fmt.Println("  --accept-status, -as    Accept page status in range (default: 200-299)")
	fmt.Println("                          Examples: 200-299  |  200-302  |  200-399")
	fmt.Println("  -r                      Follow redirects (default: off)")
	fmt.Println("                          Without -r, a 3xx response aborts with the status code")
	fmt.Println()
